...
<body>
  <div class="container">
    ...
    <div class="actions-bar">
      <button type="button" id="copyAllBtn">Copy all URLs</button>
    </div>
  ...
  <script>
    ...
    copyAllBtn.addEventListener('click', async function () {
      const links = resultsEl.querySelectorAll('.video-url');
...
// Parse a channel URL into { type, value }
function parseChannelIdentifier(url) {
  try {
    ...
    const handleSegment = segments.find(function (seg) {
...
app.get('/api/videos', async (req, res) => {
  try {
    ...
    const filtered = details.filter(function (video) {
    ...
    const videos = filtered.map(function (v) {
      return {
        id: v.id,
        title: v.title,
